...
from data_processors.wgan.helpers import prepare_data_torch_scaling_orderings
...
def get_args():
    ...
    args.add_argument("model_type", type=str, choices=['wgan', 'tablegan', 'ctgan', 'octgan', "tvae", "goggle", "tabddpm"])
...
def get_ranking(args, scores):
    ...
    for i, feat_id in enumerate(ranked):
        print(f'Rank {i}: y_{feat_id} (column {args.feats[feat_id]}) with {args.metric} score {scores[feat_id]}')
        computed_ordering.append(f'y_{feat_id}')

    ordering = '"' + " ".join(computed_ordering) + '"'
    print(f'\n ordering \n{ordering}')
...
def get_kl_ordering_botnet(args, real_data_, gen_data_, eps=1e-12):
    ...
    if args.model_type in ['tablegan']:
        ...
        if args.use_case in ['url']:
            ...
            num_perm = 5
            num_partitions = 5
    elif args.model_type=='tabddpm':
        if args.use_case in ['url', 'news']:
            num_perm = 5
            num_partitions = 3
        elif args.use_case == 'faults':
            num_perm = 5
...
def get_loaded_wasserstein_ordering(args, num_seeds):
    dataset_info = _load_json("datasets_info.json")
    # model_paths = _load_json(f"./reeval_path_{args.model_type}_dist.json")[args.model_type]
...
def get_synthetic_data_wgan(args):
    X_train, (cat_cols, cat_idx), (roundable_idx, round_digits) = read_csv(f"../C-GAN/data/{args.use_case}/train_data.csv", args.use_case)
    train_data_cols = X_train.columns

    dataset_info = _load_json("datasets_info.json")
    bin_cols_idx = dataset_info[args.use_case]["manual_inspection_categorical_cols_idx"]

    # scale data
    scaled_X_train = prepare_data_torch_scaling_orderings(X_train, args.use_case, True, bin_cols_idx, scale=(0,1))
    ...
    num_scaled_cols = scaled_X_train.shape[1]
    ...
    for i in range(args.num_sampling_rounds):
        gen_data = sample(args, generator, syn_data_shape, num_scaled_cols, None, None)
        gen_data = pd.DataFrame(gen_data, columns=train_data_cols).astype(real_data.dtypes)
        pkl.dump(gen_data, open(f'{args.model_dir}/syn_data_{args.real_data_partition}_i.pkl', 'wb'))
...
def get_synthetic_data(args):
    if args.model_type == 'wgan':
        return get_synthetic_data_wgan(args)
...
def load_saved_gen_data(args):
    ...
    elif args.model_type in ["goggle", "tabddpm"]:
        poss = {'train':0, 'val':1, 'test':2}
...
if __name__ == "__main__":
    ...
    elif args.model_type in ["tablegan", "ctgan", 'octgan', "tvae", "goggle", "tabddpm"]:
    ...
    for gen_data_ in tqdm(all_gen_data):
        if args.metric == 'kde':
            if (args.model_type != 'octgan' and args.use_case in ('botnet', 'faults')) or (args.use_case in ['faults'] and args.model_type == 'octgan')\
                    ...
                    or (args.model_type == 'goggle' and args.use_case in ('url'))\
                    or (args.model_type == 'tabddpm' and args.use_case in ('faults', 'url', 'wids', 'news')): # or (args.model_type == 'tvae' and args.use_case in ('lcld')) :
    ...
    # Store ordering in json file
    ordering = " ".join(final_ranking)
    print('Final ordering is:\n', ordering)

    # json_filename = f'feature_ordering/feature_orderings_{args.use_case}_{args.model_type}_{args.metric}_partition-{args.real_data_partition}.json'
    ...
    if json_filename.exists():
        ...
        if len(data) == 0:
            data = {}
            data["feature_orderings"] = {}
    else:
        data = {}
        data["feature_orderings"] = {}

    if args.use_case in data["feature_orderings"]:
        pass
    else:
        data["feature_orderings"][args.use_case] = {}

    if args.model_type in data["feature_orderings"][args.use_case]:
        pass
    else:
        data["feature_orderings"][args.use_case][args.model_type] = {}

    if args.metric in data["feature_orderings"][args.use_case][args.model_type]:
        pass
    else:
        data["feature_orderings"][args.use_case][args.model_type][args.metric] = {}

    if args.real_data_partition in data["feature_orderings"][args.use_case][args.model_type][args.metric]:
        pass
    else:
        data["feature_orderings"][args.use_case][args.model_type][args.metric][args.real_data_partition] = ""

    print('OLD VALUE IN JSON:\n', data["feature_orderings"][args.use_case][args.model_type][args.metric][args.real_data_partition])
    data["feature_orderings"][args.use_case][args.model_type][args.metric][args.real_data_partition] = ordering

    if len(ordering.split()) == len(data["feature_orderings"][args.use_case]['wgan']['corr'][args.real_data_partition].split()):
        with open(json_filename, "w") as f:
            print('Updated json')
            json.dump(data, f, indent=2)
    else:
        print('Did NOT update json')

# Example run command:
# python root_repo/feature_ordering/feature_ordering.py
# wids
# wasserstein
# test
# tabddpm
# --gen_data_path=../C-GAN/feature_ordering/unconstrained_models/wids/TabDDPM/INFERENCE_generated_data.pkl
# --order=ascending