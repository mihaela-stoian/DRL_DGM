...
def main():
    ...
    if args.runtime_evaluation_only:
        ...
        for i in range(num_sampling_rounds):
            start = timer()
            sampled_data, unconstrained_output = model.sample(size)
            end = timer()
            runtime = end - start
            runs.append(runtime)
        ...
        if not args.skip_evaluation: 
            ...
            wandb_project_name = f"DRL_evaluation_{args.model_type}_{args.use_case}"
            if "hyper" in args.wandb_project or "hyerparam" in args.wandb_project:
