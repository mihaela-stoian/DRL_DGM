...
def _parse_args():
    ...
    parser.add_argument('--random_dim', type=int, default=100, help='')
...
def main():
    ...
    if args.load:
        ...
        model = TableGAN(X_test,
            ...
            epochs=args.epochs, path=path, bin_cols_idx=cat_idx, warmup=args.warmup, version=args.version)
    ...
    if args.runtime_evaluation_only:
        ...
        for i in range(num_sampling_rounds):
            ...
            sampled_data = model.sample_unconstrained(size,  X_train.shape[1])
            end = timer()
            runtime = end - start
            runs.append(runtime)
